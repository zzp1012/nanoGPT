...
# data
dataset = 'ctw_topic_high_quality'
gradient_accumulation_steps = 5 * 8 # used to simulate larger batch sizes
batch_size = 12 # if gradient_accumulation_steps > 1, this is the micro-batch size
block_size = 1024
vocab_size = 65536 # np.max(dataset) + 1
# model
...
beta1 = 0.90
beta2 = 0.95
epsilon = 1e-8 # epsilon for AdamW optimizer
...
dataset = np.memmap(os.path.join(data_dir, 'pretrain_text_document.bin'), dtype=np.uint16, mode='r')
train_shuffled_idx = np.load(os.path.join(data_dir, 'pretrain_text_document_train20250624_2epoch_8192sl_44seed_shuffle_idx_sample.npy'), allow_pickle=True)
valid_shuffled_idx = np.load(os.path.join(data_dir, 'pretrain_text_document_valid20250624_10epoch_8192sl_44seed_shuffle_idx_sample.npy'), allow_pickle=True)

current_train_idx, current_valid_idx = 0, 0
def get_batch(split):
    # We recreate np.memmap every batch to avoid a memory leak, as per
    # https://stackoverflow.com/questions/45132940/numpy-memmap-memory-usage-want-to-iterate-once/61472122#61472122
    global current_train_idx, current_valid_idx
    ix_list = []
    for _ in range(ddp_world_size):
        if split == 'train':
            ix_list.append(train_shuffled_idx[current_train_idx:current_train_idx+batch_size])
            current_train_idx += batch_size
        else:
            ix_list.append(valid_shuffled_idx[current_valid_idx:current_valid_idx+batch_size])
            current_valid_idx += batch_size
    x = torch.stack([torch.from_numpy((dataset[i:i+block_size]).astype(np.int64)) for i in ix_list[ddp_rank]])
    y = torch.stack([torch.from_numpy((dataset[i+1:i+1+block_size]).astype(np.int64)) for i in ix_list[ddp_rank]])
    if device_type == 'cuda':
...
], lr=learning_rate, betas=(beta1, beta2), weight_decay=weight_decay, eps=epsilon)
