...
wandb_run_name = '0.25B_dense' # 'run' + str(time.time())
# data
dataset = 'openwebtext'
...
# merge settings
merge_prob = 0.1 # probability of merging two experts
merge_interval = 100 # how often to merge experts
# DDP settings
...
optimizer = torch.optim.AdamW(optim_groups, lr=learning_rate, betas=(beta1, beta2), fused=use_fused)
...
# extract moe modules
def extract_moe_modules_by_attr(model, attr="experts"):
    """
    Extract all modules with the given attribute from the model.
    """
    moe_modules = []
...
def merge_two_experts(moe_module, seed=0):
    ...
    idx_a, idx_b = random.Random(seed).sample(range(num_experts), 2)
    expert_a = experts[idx_a]
    expert_b = experts[idx_b]
    for p_a, p_b in zip(expert_a.parameters(), expert_b.parameters()):
