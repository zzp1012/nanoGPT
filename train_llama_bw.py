...
from model_llama import build_llama_model
...
wandb_run_name = '0.25B_baseline' # 'run' + str(time.time())
# data
dataset = 'openwebtext'
...
embed_param = [p for n, p in param_dict.items() if 'embed' in n]
...
mlp_param = [p for n, p in param_dict.items() if 'mlp' in n]
ln_param = [p for n, p in param_dict.items() if 'norm' in n]
head_param = [p for n, p in param_dict.items() if 'lm_head' in n]
